...
OUT_EXP = Regex("out ($REGISTER_EXP)")

function parseOut(text::AbstractString)::Optional{Out}
    m = match(OUT_EXP, text)
    if m !== nothing
        val = m.captures[1][1]
        return Out(val)
    end
...
function parseInstruction(text::AbstractString)::Instruction
    for f in (parseCpy, parseDec, parseInc, parseJnz, parseTgl, parseOut)
        instruction = f(text)
...
mutable struct State
    ...
    lastOutput::Optional{Int}
    previousOutStates::Set{Tuple}
end
...
function exec!(instruction::Out, state::State)
    val = valueOf(instruction.val, state)
    if val ∉ (0, 1) || val == state.lastOutput
        return :badsignal
    end
    registers = Tuple(values(state.registers))
    if registers ∉ state.previousOutStates
        push!(state.previousOutStates, registers)
        state.lastOutput = val
    else # we are in an infinite loop
        if state.lastOutput !== nothing
            return :goodsignal
        else
            return :badsignal
        end
...
function step!(state::State)
    ...
    res = exec!(instruction, state)
    if res ∈ (:goodsignal, :badsignal)
        return res
    end
end

function run(instructions::Vector{Instruction}, a::Int)
    state = State(
        1, 
        Dict(
            'a' => a,
