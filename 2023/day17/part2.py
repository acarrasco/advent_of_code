...
def simulate(problem):
    ...
    def neighbors(state):
        ...
        if not d:
            for di, dj in [SOUTH, EAST]:
                yield State(i=i+di, j=j+dj, d=(di, dj), s=1)
            return
        ...
        if s >= MIN_STRAIGHT_STEPS:
            # left
            li, lj = LEFT[d]
            ni, nj = i + li, j + lj
            if inside(ni, nj):
                yield State(i=ni, j=nj, d=(li, lj), s=1)
            # right
            ri, rj = RIGHT[d]
            ni, nj = i + ri, j + rj
            if inside(ni, nj):
                yield State(i=ni, j=nj, d=(ri, rj), s=1)
    ...
    def is_goal(state):
        return state.i == rows - 1 and state.j == cols - 1 and state.s >= MIN_STRAIGHT_STEPS
