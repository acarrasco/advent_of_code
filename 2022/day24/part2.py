...
def solve(reachable, rows, cols):
    ...
    def is_goal(goal_i, goal_j):
        def g(node):
            i, j, _ = node
            return i == goal_i and j == goal_j
        return g

    def get_h(goal_i, goal_j):
        def h(node):
            i, j, _ = node
    ...
    first_leg = a_star((-1, 0, 0), is_goal(rows, cols - 1), neighbors, get_h(rows, cols - 1))
    second_leg = a_star((rows, cols-1, first_leg % period), is_goal(-1, 0), neighbors, get_h(-1, 0))
    third_leg = a_star((-1, 0, (first_leg + second_leg) % period), is_goal(rows, cols - 1), neighbors, get_h(rows, cols - 1))
    return first_leg + second_leg + third_leg
